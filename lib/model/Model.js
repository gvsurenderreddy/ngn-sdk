...
var Class = NGN.Class.extend({
  ...
  /**
    ...
    Class.super.constructor.call(this,config);
      ...
      modified: {
        ...
        writable:	true
			 ...
			 */
			allowInvalidSave: {
				value: NGN.coalesce(config.allowInvalidSave,false),
				enumerable:	false,
				writable:	true
			},
			...
			disableDataValidation: {
				value: NGN.coalesce(config.disableDataValidation,false),
				enumerable:	false,
				writable:	true
			},
    ...
    // Auto-fetch
		...
		//TODO: Support for global notifications
		/*if (NGNA){
   ...
   * The name of the data field.
	...
	serialize: function(obj,serializeAssociations){
		...
		serializeAssociations = NGN.coalesce(serializeAssociations,false);
		...
		for (var key in _obj) {
			...
			if (this.fields.hasOwnProperty(key)) {
				...
				if ((_obj.hasOwnProperty(key) && (_obj.nonEnumerableProperties.indexOf(key) < 0 && /^[a-z0-9 ]$/.test(key.substr(0,1)))) || (_obj[key] !== undefined && _obj.enumerableProperties.indexOf(key) >= 0)) {
					...
					if (dsc.get && !this.fields[key] instanceof NGN.model.data.Association){
						try {
								...
								if (!isXRef) {
									if (Array.isArray(_obj[key])){
										for (var i=0;i<_obj[key].length;i++){
											if (_obj[key][i] instanceof NGN.model.data.Association){
	...
	extension: function() {
		...
		if (this.isXtn || !UTIL.PROXY_SUPPORT)
			return this;
   ...
   * The JSON change object.
	...
	onBeforeChange: function(change) {
		...
		if (change !== null)
			this.emit('beforeChange'+NGN.string.capitalize(change.property),change);
		return true;
	},
	...
	onChange: function(change) {
		...
		if (change !== null)
			this.emit('change'+NGN.string.capitalize(change.property),change);
	},
